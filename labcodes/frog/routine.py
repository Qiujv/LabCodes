...
class StateDis_KMeans:
    ...
    Examples:
    ```
        i0 = np.random.normal(loc= 1, scale=0.3, size=500)
        q0 = np.random.normal(loc= 1, scale=0.3, size=500)
        i1 = np.random.normal(loc= 1, scale=0.3, size=500)
        q1 = np.random.normal(loc=-1, scale=0.3, size=500)
        i2 = np.random.normal(loc=-1, scale=0.3, size=500)
        q2 = np.random.normal(loc= 1, scale=0.3, size=500)
        iqs = [i0 + 1j*q0, i1 + 1j*q1, i2 + 1j*q2]
        stater, ax, ax2 = StateDis_KMeans.fit(iqs, plot=True)

        stater = StateDis_KMeans(centers=[1+1j, 1-1j, -1+1j])
        ...
        stater.judge(iqs[0]), stater.probs(iqs[1])
    ```
    """
    ...
    def fit(cls, list_points:list, plot:bool = False):
        ...
        Args:
            list_points: list of complex points for different state preparations. e.g.
                `[arr_c0, arr_c1, arr_c2, ...]`
        ...
        kms = KMeans(n_clusters=n_clusters, n_init='auto').fit(flatten_pts)
        # centers = kms.cluster_centers_  # The order is random, DO NOT use.
    ...
    def judge(self, points:np.array):
        """Returns state flags from given complex points.

        Returns:
            `[0, 2, 1, 1, 0, ...]`
        """
        n_states = len(self.centers)
        state_flags = np.argmin(np.abs(points - self.centers.reshape((n_states, 1))), axis=0)
        return state_flags
    
    def probs(self, points:np.array):
        """Returns state probabilities from given complex points.

        Returns:
            Array with length same as centers, e.g. `[0.5, 0.02, 0.01, 0.47]`.
        """
        flags = self.judge(points)
    ...
    def plot_regions(self, ax:plt.Axes):
        ...
        xx, yy = np.meshgrid(
            np.linspace(xmin, xmax, 201), 
            np.linspace(ymin, ymax, 201)
        )
        ax.imshow(
            self.judge(xx.ravel() + 1j*yy.ravel()).reshape(xx.shape),
